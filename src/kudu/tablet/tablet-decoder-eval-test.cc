...
class TabletDecoderEvalTest : public KuduMultiTabletTest,
...
public:
  ...
  TabletDecoderEvalTest()
    : KuduMultiTabletTest(Schema({ColumnSchema("key", INT32),
                                  ColumnSchema("int_val", INT32),
                                  ColumnSchema("string_val", STRING)}, 1), 13),
      cardinalities_({5, 10, 15, 20, 30, 40, 60, 80, 100, 200, 400, 500, 1000}),
      num_tablets_(cardinalities_.size()) {
  }

  virtual void SetUp() OVERRIDE {
    LOG(INFO) << "SetUp() called from TabletDecoderEvalTest";
    KuduMultiTabletTest::SetUp();

    FillTestTablets();
  }

  void FillTestTablets() {
    ...
    for (int t = 0; t < cardinalities_.size(); t++) {
      // Create new tablet/replace old tablet
      LocalTabletWriter writer(tablet(t).get(), &client_schema_);
      KuduPartialRow row(&client_schema_);
      const size_t cardinality = cardinalities_[t];
  ...
  void TestScanYieldsExpectedResults(ScanSpec spec, uint32_t pred_upper) {
    ...
    for (size_t i = 0; i < cardinalities_.size(); i++) {
      ...
      size_t expected_sel_count = pred_upper > num_tablets_ ?
                               nrows_ :
                               nrows_ / pred_upper * cardinalities_[i] + nrows_ % cardinalities_[i];
      gscoped_ptr<RowwiseIterator> iter;
      ASSERT_OK(tablet(i)->NewRowIterator(client_schema_, &iter));
  ...
  void TestCountOnlyScanYieldsExpectedResults(ScanSpec spec) {
    ...
    gscoped_ptr <RowwiseIterator> iter;
    ASSERT_OK(tablet(0)->NewRowIterator(empty_schema, &iter));
    ASSERT_OK(iter->Init(&spec));
    ASSERT_TRUE(spec.predicates().empty()) << "Should have accepted all predicates";
