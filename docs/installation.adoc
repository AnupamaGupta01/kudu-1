...
`thirdparty/build-if-necessary.sh`, you may run into output similar to the following:
...
dependencies. Completely remove them and reinstall the correct dependencies.

[source,bash]
----
$ git clean -fdx
$ brew uninstall autoconf automake cmake libtool pkg-config boost pstree
$ brew install autoconf automake cmake libtool pkg-config boost pstree
----

Continue starting from Step 3.
====
.OSX Build Script
====
